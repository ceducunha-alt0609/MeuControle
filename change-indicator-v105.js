/* MeuControle — Indicador remoto V1.05: baseline limpo + contagem real + ordem fixa Sino > Sync > Perfil */
(()=>{
  if(window.__mcChangeIndicatorV105)return;window.__mcChangeIndicatorV105=true;
  window.__mcChangeIndicatorV104=true;window.__mcChangeIndicatorV103=true;
  const VERSION='1.05',DEVICE_KEY='meu_controle_device_id_v1',STATE_PREFIX='meu_controle_remote_indicator_v5:';
  ...
  function style(){if(document.getElementById('mcChangeIndicatorStyleV105'))return;const st=document.createElement('style');st.id='mcChangeIndicatorStyleV105';st.textContent=`
  ...
  function ensureUi(){
    ...
    if(!bell){bell=document.createElement('button');bell.type='button';bell.className='sync-new-bell';bell.dataset.mcBell='v105';bell.innerHTML='<span aria-hidden="true">🔔</span><span class="sync-new-bell-count">0</span>'}
    placeBell(bell);
