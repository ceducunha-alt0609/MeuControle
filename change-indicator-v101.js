...
(()=>{
  ...
  function start(){const s=session(),ws=workspace();if(!s?.signedIn||!s.uid||!ws?.watch){stop();render();retryTimer=setTimeout(start,500);return}if(watchUid===s.uid&&unsub)return;stop();watchUid=s.uid;unsub=ws.watch(s.uid,remote=>{const t=parseTime(remote?.clientUpdatedAt);if(!t)return;const key=seenKey(s.uid),seen=Number(localStorage.getItem(key)||0);if(!seen){localStorage.setItem(key,String(t));return}if(t<=seen)return;localStorage.setItem(key,String(t));if(!remote?.sourceDeviceId||remote.sourceDeviceId===deviceId())return;lastResult={count:lastResult.count+1,lastAt:t};render()},()=>{})}
