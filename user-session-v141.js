/* MeuControle — V1.41: identidade/sessão do usuário sem tocar nos dados */
(()=>{
  ...
  function style(){
    ...
    const st=document.createElement('style');st.id='mcUserSessionV141Style';st.textContent=`
      ...
      .mc-user-session-actions-v141{display:flex;gap:8px;margin-top:12px}.mc-user-session-actions-v141 button{min-height:40px}.mc-user-session-note-v141{margin-top:11px;padding:9px 10px;border-radius:10px;background:#fff7df;color:#7b6420;font-size:10px;line-height:1.45}
      ...
      body.mc-dark .mc-user-session-v141{background:#1b262d;border-color:#314049}.mc-dark .mc-user-session-head-v141 p,.mc-dark .mc-user-session-copy-v141 span,.mc-dark .mc-profile-scope-note-v141{color:#9eabb3!important}.mc-dark .mc-user-session-note-v141{background:#3b3520;color:#f1da87}
  ...
  function ensure(){
    ...
    const card=dataCard();if(card&&!card.querySelector('.mc-user-session-v141')){
      const box=document.createElement('section');box.className='mc-user-session-v141';box.innerHTML=`
        ...
        <div class="mc-user-session-note-v141">Ciclo 1: sua identidade já está separada dos perfis, mas os lançamentos ainda continuam locais e não são separados por usuário até o próximo ciclo.</div>`;
  ...
  function render(){
    ...
    if(state.signedIn){
      ...
      badge.textContent='Modo local';badge.classList.remove('on');name.textContent='Sem conta conectada';mail.textContent='Os dados continuam apenas neste navegador.';login.hidden=false;sw.hidden=true;out.hidden=true;avatar.textContent='?';
    }
  }
