/* MeuControle — Indicador remoto V1.03: deduplicação por conteúdo + posição correta */
(()=>{
  if(window.__mcChangeIndicatorV103)return;window.__mcChangeIndicatorV103=true;
  const VERSION='1.03',DEVICE_KEY='meu_controle_device_id_v1',STATE_PREFIX='meu_controle_remote_indicator_v3:';
  let unsub=null,watchUid=null,retryTimer=null,lastResult={count:0,lastAt:0};
  ...
  function style(){if(document.getElementById('mcChangeIndicatorStyleV103'))return;const st=document.createElement('style');st.id='mcChangeIndicatorStyleV103';st.textContent=`
    ...
    @media(min-width:701px){.desktop-top-tools-v139 .sync-new-bell{position:relative!important;right:auto!important;top:auto!important;margin:0!important;flex:0 0 42px}}
  ...
  function placeBell(bell){
    ...
    if(matchMedia('(min-width:701px)').matches){
      ...
      if(tools){const profile=tools.querySelector('.desktop-profile-btn-v139');if(bell.parentElement!==tools||bell.nextElementSibling!==profile)tools.insertBefore(bell,profile||null);return}
    }
    const brand=document.querySelector('.topbar .brand');if(brand&&bell.parentElement!==brand)brand.appendChild(bell);
  }
  function ensureUi(){
    style();let bell=document.querySelector('.sync-new-bell');
    if(!bell){bell=document.createElement('button');bell.type='button';bell.className='sync-new-bell';bell.innerHTML='<span aria-hidden="true">🔔</span><span class="sync-new-bell-count">0</span>'}
    placeBell(bell);
  ...
  function start(){
    ...
    unsub=ws.watch(s.uid,remote=>{
      ...
      if(!prev.hash){saveState(uid,{hash,workspace:clone(remote.workspace),time:t});return}
  ...
  window.addEventListener('meucontrole:user-workspace-imported',()=>setTimeout(()=>{render();placeBell(document.querySelector('.sync-new-bell'))},50));
  ...
  setTimeout(()=>{ensureUi();render();start()},450);setTimeout(()=>placeBell(document.querySelector('.sync-new-bell')),1800);
