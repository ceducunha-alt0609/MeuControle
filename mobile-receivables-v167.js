/* Meu Controle — V1.72: recebimentos seguem o mesmo padrão visual das despesas */
(()=>{
 ...
 function css(){if(document.getElementById('mcMobileReceivablesV167Style'))return;const s=document.createElement('style');s.id='mcMobileReceivablesV167Style';s.textContent=`@media(max-width:700px){
  ...
  #dashboardDetailModal.mc-receivables-detail-v172 #dashboardDetailList .item-value{color:#13713d!important}
  ...
  body.mc-dark #dashboardDetailModal.mc-receivables-detail-v172 #dashboardDetailList .item-value{color:#71d79b!important}
 ...
 function showDetails(){const v=values(),modal=document.getElementById('dashboardDetailModal'),title=document.getElementById('dashboardDetailTitle'),sub=document.getElementById('dashboardDetailSubtitle'),box=document.getElementById('dashboardDetailList');if(!modal||!title||!sub||!box)return;modal.classList.remove('mc-receivables-detail-v171');modal.classList.add('mc-receivables-detail-v172');title.textContent='A receber no mês';sub.textContent=`${v.month.length} recebimento${v.month.length===1?'':'s'} pendente${v.month.length===1?'':'s'} • ${money(v.totalMonth)}`;box.innerHTML='';if(!v.month.length){box.innerHTML='<div class="empty">Nenhum recebimento pendente neste mês.</div>'}else v.month.slice().sort((a,b)=>(a.date+(a.time||'')).localeCompare(b.date+(b.time||''))).forEach(e=>{const node=createItemNode(e,'dashboard');box.appendChild(node);const item=node?.classList?.contains('item')?node:node?.querySelector?.('.item');if(!item)return;const value=item.querySelector('.item-value,.value');if(value)value.classList.add('item-value');const actions=item.querySelector('.actions');if(actions){[...actions.querySelectorAll('button')].forEach(btn=>{const t=(btn.textContent||'').trim().toLowerCase();if(t.includes('duplic')||t.includes('excluir'))btn.remove()})}});modal.classList.remove('hidden')}
