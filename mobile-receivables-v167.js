/* Meu Controle — V1.74: recebimentos com ações alinhadas à direita */
(()=>{
 ...
 function css(){if(document.getElementById('mcMobileReceivablesV167Style'))return;const s=document.createElement('style');s.id='mcMobileReceivablesV167Style';s.textContent=`@media(max-width:700px){
  ...
  #dashboardPage .dashboard-hint{display:none!important}
  #dashboardDetailModal.mc-receivables-detail-v174 #dashboardDetailList .item-value{color:#13713d!important}
  #dashboardDetailModal.mc-receivables-detail-v174 #dashboardDetailList .actions button:nth-child(n+3){display:none!important}
  #dashboardDetailModal.mc-receivables-detail-v174 #dashboardDetailList .item{overflow:hidden!important}
  #dashboardDetailModal.mc-receivables-detail-v174 #dashboardDetailList .actions{left:auto!important;right:12px!important;max-width:212px!important;overflow:visible!important;justify-content:flex-end!important}
  ...
  body.mc-dark #dashboardDetailModal.mc-receivables-detail-v174 #dashboardDetailList .item-value{color:#71d79b!important}
 ...
 function keepPrimaryActions(root){if(!root)return;const buttons=[...root.querySelectorAll('button')];buttons.forEach(btn=>{const t=(btn.textContent||'').trim().toLowerCase();if(t.includes('duplic')||t.includes('excluir'))btn.remove()})}
 function showDetails(){const v=values(),modal=document.getElementById('dashboardDetailModal'),title=document.getElementById('dashboardDetailTitle'),sub=document.getElementById('dashboardDetailSubtitle'),box=document.getElementById('dashboardDetailList');if(!modal||!title||!sub||!box)return;modal.classList.remove('mc-receivables-detail-v171','mc-receivables-detail-v172','mc-receivables-detail-v173');modal.classList.add('mc-receivables-detail-v174');title.textContent='A receber no mês';sub.textContent=`${v.month.length} recebimento${v.month.length===1?'':'s'} pendente${v.month.length===1?'':'s'} • ${money(v.totalMonth)}`;box.innerHTML='';if(!v.month.length){box.innerHTML='<div class="empty">Nenhum recebimento pendente neste mês.</div>'}else v.month.slice().sort((a,b)=>(a.date+(a.time||'')).localeCompare(b.date+(b.time||''))).forEach(e=>{const node=createItemNode(e,'dashboard');box.appendChild(node);const value=node?.querySelector?.('.item-value,.value');if(value)value.classList.add('item-value');keepPrimaryActions(node)});keepPrimaryActions(box);requestAnimationFrame(()=>keepPrimaryActions(box));modal.classList.remove('hidden')}
