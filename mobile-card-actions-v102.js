...
(()=>{
  ...
  let activeCard=null,lastFocus=null;
  ...
  function installStyle(){
    ...
    const st=document.createElement('style');st.id='mcMobileCardActionsV102Style';st.textContent=`
      ...
      @media(max-width:700px){
        ...
        #launchesPage.mobile-launch-list .item.mc-mobile-compact-v102:focus-visible{outline:3px solid rgba(var(--primary-rgb),.28);outline-offset:2px}
        #launchesPage.mobile-launch-list .item.mc-mobile-compact-v102 .item-main{min-width:0!important;width:100%!important}
        #launchesPage.mobile-launch-list .item.mc-mobile-compact-v102 .item-title-row{display:grid!important;grid-template-columns:minmax(0,1fr) auto auto!important;gap:7px!important;align-items:center!important;min-width:0!important}
        #launchesPage.mobile-launch-list .item.mc-mobile-compact-v102 .item-title{margin:0!important;min-width:0!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;font-size:18px!important;line-height:1.25!important}
        ...
        #launchesPage.mobile-launch-list .item.mc-mobile-compact-v102 .mc-mobile-inline-amount-v102{margin:0!important;padding-left:4px!important;white-space:nowrap!important;font-size:17px!important;line-height:1.2!important;color:var(--primary-dark)!important;text-align:right!important}
        #launchesPage.mobile-launch-list .item.mc-mobile-compact-v102 .mc-mobile-inline-amount-v102:empty{display:none!important}
        #launchesPage.mobile-launch-list .item.mc-mobile-compact-v102 .meta{margin-top:5px!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;font-size:12px!important;line-height:1.35!important}
        #launchesPage.mobile-launch-list .item.mc-mobile-compact-v102 .notes{display:block!important;height:17px!important;min-height:17px!important;margin-top:4px!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;font-size:11px!important;line-height:17px!important;color:#7d8882!important}
        ...
        body.mc-dark .mc-card-action-delete-v102{background:#a53b3b!important;border-color:#b94a4a!important;color:#fff!important}.mc-card-actions-notes-v176,.mc-card-actions-steps-v176{margin:0 0 14px;padding:13px 15px;border:1px solid #dce6e1;border-radius:13px;background:#f7faf8;text-align:left}.mc-card-actions-notes-v176[hidden],.mc-card-actions-steps-v176[hidden]{display:none!important}.mc-card-actions-notes-v176 b,.mc-card-actions-steps-v176 b{display:block;margin-bottom:7px;font-size:11px;color:#64766d;text-transform:uppercase;letter-spacing:.04em}.mc-card-action-step-v176{display:grid;grid-template-columns:22px minmax(0,1fr);align-items:center;gap:10px;padding:7px 0;font-size:14px;line-height:1.35;text-align:left}.mc-card-action-step-v176 input{width:18px;height:18px;margin:0;accent-color:var(--primary)}.mc-card-action-step-v176 span{min-width:0;color:inherit}.mc-card-action-step-v176.done span{text-decoration:line-through;opacity:.55}body.mc-dark .mc-card-actions-notes-v176,body.mc-dark .mc-card-actions-steps-v176{background:#202b32;border-color:#43545e;color:#e8f0f3}body.mc-dark .mc-card-actions-notes-v176 b,body.mc-dark .mc-card-actions-steps-v176 b{color:#c6d2d8!important}.mc-card-action-step-v176 span{color:#d9e3e7!important}.mc-card-action-step-v176.done span{color:#9eabb1!important;opacity:.72}
  ...
  function closeSheet(){
    const bd=document.querySelector('.mc-card-actions-backdrop-v102');if(bd)bd.hidden=true;
    activeCard=null;
    const f=lastFocus;lastFocus=null;
    if(f&&document.contains(f))requestAnimationFrame(()=>f.focus({preventScroll:true}));
  }
  function ensureSheet(){
    ...
    bd.innerHTML='<section class="mc-card-actions-sheet-v102" role="dialog" aria-modal="true" aria-label="Ações do lançamento"><div class="mc-card-actions-head-v102"><div class="mc-card-actions-copy-v102"><strong></strong><span></span></div><button type="button" class="mc-card-actions-close-v102" aria-label="Fechar">×</button></div><div class="mc-card-actions-notes-v176" hidden><b>Observações</b><div></div></div><div class="mc-card-actions-steps-v176" hidden><b>Etapas</b><div></div></div><div class="mc-card-actions-grid-v102"><button type="button" data-card-action="edit" class="secondary-action">Editar</button><button type="button" data-card-action="done">Concluir</button><button type="button" data-card-action="duplicate" class="secondary-action">Duplicar</button><button type="button" data-card-action="delete" class="mc-card-action-delete-v102">Excluir</button></div></section>';
    ...
    bd.addEventListener('keydown',e=>{
      ...
      if(e.key!=='Tab')return;
      const focusables=[...bd.querySelectorAll('button:not([hidden]):not(:disabled)')];if(!focusables.length)return;
  ...
  function runAction(action){
    ...
    const bd=document.querySelector('.mc-card-actions-backdrop-v102');if(bd)bd.hidden=true;activeCard=null;lastFocus=null;
  ...
  function openSheet(card){
    ...
    activeCard=card;lastFocus=document.activeElement;const bd=ensureSheet(),title=card.querySelector('.item-title')?.textContent?.trim()||'Lançamento',meta=card.querySelector('.meta')?.textContent?.trim()||'';
    bd.querySelector('.mc-card-actions-copy-v102 strong').textContent=title;bd.querySelector('.mc-card-actions-copy-v102 span').textContent=meta;const entry=(()=>{try{return entries.find(x=>String(x.id)===String(card.dataset.entryId))}catch{return null}})(),notes=entry?.notes?.trim?.()||'';const nb=bd.querySelector('.mc-card-actions-notes-v176');if(nb){nb.hidden=!notes;nb.querySelector('div').textContent=notes}const sb=bd.querySelector('.mc-card-actions-steps-v176');if(sb){const list=sb.querySelector('div');list.innerHTML='';const steps=Array.isArray(entry?.steps)?entry.steps:[];sb.hidden=!steps.length;steps.forEach(step=>{const row=document.createElement('label');row.className='mc-card-action-step-v176'+(step.done?' done':'');row.innerHTML='<input type="checkbox" '+(step.done?'checked':'')+'><span></span>';row.querySelector('span').textContent=step.text;row.querySelector('input').onchange=ev=>{step.done=ev.target.checked;row.classList.toggle('done',step.done);try{save();window.MeuControleUserDataScope?.saveCurrent?.()}catch{}};list.appendChild(row)})}
    ...
    bd.hidden=false;requestAnimationFrame(()=>bd.querySelector('.mc-card-actions-close-v102')?.focus({preventScroll:true}));
  }
  ...
  function decorateFragment(fragment){
    ...
    card.classList.add('mc-mobile-compact-v102');card.setAttribute('tabindex','0');card.setAttribute('role','button');card.setAttribute('aria-label','Abrir ações deste lançamento');
    ...
    card.addEventListener('keydown',e=>{if(!mobile()||e.target.closest('button,input,label,a'))return;if(e.key==='Enter'||e.key===' '){e.preventDefault();openSheet(card)}});
    return fragment;
  }
  ...
  function redecorate(){
    ...
    document.querySelectorAll('#launchesPage #list .item').forEach(card=>{
      ...
      if(!card.__mcActionsBoundV102){card.__mcActionsBoundV102=true;card.addEventListener('click',e=>{if(!mobile()||e.target.closest('button,input,label,a')||document.body.classList.contains('mc-bulk-active-mobile'))return;openSheet(card)});card.addEventListener('keydown',e=>{if(!mobile()||e.target.closest('button,input,label,a'))return;if(e.key==='Enter'||e.key===' '){e.preventDefault();openSheet(card)}})}
    })
  }
