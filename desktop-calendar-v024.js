/* Meu Controle — V0.28: calendário desktop premium + competência única */
(function(){
 ...
 @media(min-width:701px){
  ...
  #calendarPage .calendar-month-btn.active{z-index:3;min-height:68px;margin:-5px -2px -5px;padding:9px 4px 7px;border-color:rgba(255,255,255,.68);border-radius:15px;background:linear-gradient(145deg,rgba(34,105,153,.94),rgba(16,72,111,.88));color:#fff;transform:translateY(-3px) scale(1.045);box-shadow:0 13px 26px rgba(22,79,120,.28),inset 0 1px 0 rgba(255,255,255,.55),inset 0 -1px 0 rgba(6,43,68,.2);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px)}
  #calendarPage .calendar-month-btn.active:hover{color:#fff;transform:translateY(-4px) scale(1.055);box-shadow:0 15px 30px rgba(22,79,120,.32),inset 0 1px 0 rgba(255,255,255,.65)}
  #calendarPage .calendar-month-btn.active::before{content:"";position:absolute;left:7%;right:7%;top:0;height:45%;border-radius:14px 14px 55% 55%;background:linear-gradient(180deg,rgba(255,255,255,.38),rgba(255,255,255,0));pointer-events:none}
  #calendarPage .calendar-month-btn.active::after{content:"";position:absolute;left:24%;right:24%;bottom:3px;height:3px;border-radius:99px;background:rgba(255,255,255,.95);box-shadow:0 0 10px rgba(255,255,255,.55)}
  #calendarPage .calendar-month-btn.active .month-name-v024{font-size:13px;font-weight:900;letter-spacing:.08em;position:relative;z-index:1;color:#fff;text-shadow:0 1px 2px rgba(0,0,0,.08)}
  #calendarPage .calendar-month-btn.active .month-badge{font-size:17px;font-weight:900;position:relative;z-index:1;color:#fff!important;text-shadow:0 1px 2px rgba(0,0,0,.08)}
 ...
 function setPeriod(m,y){calendarMonth=m;calendarYear=y;try{dashboardMonth=m;dashboardYear=y;renderDashboard()}catch{}renderCalendar();requestAnimationFrame(()=>window.MeuControleDesktopPendingExpensesV163?.refresh?.())}
 function desktopRender(){styles();$('calendarYearLabel').textContent=calendarYear;const months=$('calendarMonths');months.innerHTML='';const yearEntries=profileFiltered(entries).filter(e=>Number(e.date.slice(0,4))===calendarYear);for(let m=0;m<12;m++){const count=yearEntries.filter(e=>Number(e.date.slice(5,7))===m+1).length;const b=document.createElement('button');b.className='calendar-month-btn'+(count?' has-events-v025':'')+(m===calendarMonth?' active':'');b.innerHTML=`<span class="month-name-v024">${shortMonths[m]}</span>${count?`<span class="month-badge">${count}</span>`:''}`;b.onclick=()=>setPeriod(m,calendarYear);months.appendChild(b)}const yc=document.createElement('div');yc.className='calendar-year-inline-v024';yc.innerHTML='<button type="button" aria-label="Ano anterior">‹</button><strong></strong><button type="button" aria-label="Próximo ano">›</button>';yc.querySelector('strong').textContent=calendarYear;const yb=yc.querySelectorAll('button');yb[0].onclick=()=>setPeriod(calendarMonth,calendarYear-1);yb[1].onclick=()=>setPeriod(calendarMonth,calendarYear+1);months.appendChild(yc);
 ...
 window.addEventListener('load',()=>setTimeout(()=>{try{renderCalendar()}catch{}},600));mq.addEventListener?.('change',()=>setTimeout(()=>{try{renderCalendar()}catch{}},50));styles();setTimeout(()=>{try{if(mq.matches)renderCalendar()}catch{}},120);window.MeuControleDesktopCalendarV024={version:'0.28'};
})();
