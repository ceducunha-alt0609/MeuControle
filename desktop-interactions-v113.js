/* MeuControle — V1.14: ações flutuantes com observações no desktop */
(function(){
  if(window.__mcDesktopInteractionsV113)return;window.__mcDesktopInteractionsV113=true;
  const VERSION='1.14',desktop=()=>matchMedia('(min-width:701px)').matches;
  ...
  function styles(){
    ...
    const s=document.createElement('style');s.id='mcDesktopInteractionsV113Style';s.textContent=`
      ...
      @media(min-width:701px){
        ...
        .mc-desktop-actions-note-v114{display:none;margin:0 4px 10px;padding:9px 10px;border:1px solid #dfe7e3;border-radius:10px;background:#f6f9f7;color:#43544b;font-size:11px;line-height:1.42;white-space:pre-wrap;overflow-wrap:anywhere}.mc-desktop-actions-note-v114.show{display:block}.mc-desktop-actions-note-v114 strong{display:block;margin-bottom:4px;color:#315142;font-size:10px;letter-spacing:.045em;text-transform:uppercase}
        ...
        body.mc-dark .mc-desktop-actions-note-v114{background:#152128;border-color:#40505a;color:#c7d2cc}body.mc-dark .mc-desktop-actions-note-v114 strong{color:#dbe7e1}
      }
  ...
  function ensurePanel(){
    ...
    p.innerHTML='<div class="mc-desktop-actions-handle-v113" title="Arraste para mover">•••</div><div class="mc-desktop-actions-head-v113"><div class="mc-desktop-actions-copy-v113"><strong></strong><span></span></div><button type="button" class="mc-desktop-actions-close-v113" aria-label="Fechar">×</button></div><div class="mc-desktop-actions-note-v114"><strong>Observações</strong><span></span></div><div class="mc-desktop-actions-grid-v113"><button type="button" data-action="edit" class="secondary-action">Editar</button><button type="button" data-action="done">Concluir</button><button type="button" data-action="duplicate" class="secondary-action">Duplicar</button><button type="button" data-action="delete" class="mc-desktop-delete-v113">Excluir</button></div>';
  ...
  function openPanel(card){
    if(!desktop()||!card)return;activeCard=card;const p=ensurePanel(),title=card.querySelector('.item-title')?.textContent?.trim()||'Lançamento',meta=card.querySelector('.meta')?.textContent?.trim()||'',notes=card.querySelector('.notes')?.textContent?.trim()||'';p.querySelector('.mc-desktop-actions-copy-v113 strong').textContent=title;p.querySelector('.mc-desktop-actions-copy-v113 span').textContent=meta;const note=p.querySelector('.mc-desktop-actions-note-v114');note.querySelector('span').textContent=notes;note.classList.toggle('show',!!notes);const done=p.querySelector('[data-action="done"]');done.textContent=card.querySelector('.doneBtn')?.textContent?.trim()||'Concluir';p.querySelector('[data-action="duplicate"]').hidden=!card.querySelector('.duplicateBtn');placePanel(card);requestAnimationFrame(()=>p.classList.add('show'));
  }
